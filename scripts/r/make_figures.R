...
## libraries
libs <- c("tidyverse", "data.table", "sf", "patchwork", "ggthemes", "maps",
          "lubridate")
...
## holc w/ FCC 477 data
df_holc_bg <- readRDS(file.path(cln_dir, "sf_bg_holc.RDS")) %>%
  st_drop_geometry()
...
preds <- readRDS(file.path(cln_dir, "predictions_fcc.RDS")) %>%
...
df_tmp <- df %>%
  ...
  distinct(group, group_) %>%
  arrange(group) %>%
  mutate(group_ = fct_reorder(group_,
                              group,
                              .fun = identity))

## rejoin
df <- df %>% left_join(df_tmp, by = "group")
...
g <- ggplot(df %>% mutate(total_zone = total_zone / 1000),
  ...
  facet_wrap(~ demo, ncol = 1, scales = "free_x") +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = round(total_zone)),
            position = position_dodge(width = 0.9),
            vjust = -0.5,
            size = 2.25) +
  scale_fill_manual("HOLC", values = holc_vals) +
